...
import {fetchPositions, forceResetUsers, signupUser, USER_REGISTERED} from "../../redux/actions/users-actions";
...
list of users in the block from the top`
  ...
  const handleNumberChange = (value) => {
    ...
    if (isNaN(strWithoutPlusAndSpaces) || value.length < 5 || !value.startsWith('+380 ')) {
      reactDispatch({
        type: FORM_INPUT_UPDATE, inputId: 'phone', value: inputValues.phone,
        isValid: false
      });
    } else {
      ...
      <div className="signup-wrapper" style={blurSettings}>
        ...
        <form>
          ...
          <input className="input" type="text" id="name" name="name" minLength="2" maxLength="60"
                 ...
                 style={(inputTouched.name && !inputValidities.name) ? {borderColor: '#f00'} : null}/>
          ...
          <input className="input" type="email" id="email" name="email" minLength="2" maxLength="60"
                 ...
                 style={(inputTouched.email && !inputValidities.email) ? {borderColor: '#f00'} : null}/>
          ...
          <input className="input" type="tel" id="phone" name="phone" minLength="17" maxLength="17"
                 ...
                 style={(inputTouched.phone && !inputValidities.phone) ? {borderColor: '#f00'} : null}/>
          ...
          <input className="photo" type="file" id="photo" name="photo" required accept="image/png, image/jpeg"
                 ...
                 style={(inputTouched.photo && !inputValidities.photo) ? {borderColor: '#f00'} : null}/>
        ...
        {!reactState.formIsValid && inputValues.email && (
          <p className="hint">Fill up all form with valid values first</p>)}
      </div>
